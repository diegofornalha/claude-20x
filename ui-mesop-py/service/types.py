...
class RemoveAgentRequest(JSONRPCRequest):
    method: Literal['agent/remove'] = 'agent/remove'
    # This is the base url of the agent to remove
    params: str | None = None


class RemoveAgentResponse(JSONRPCResponse):
    result: str | None = None
...
class AgentStatus(str, Enum):
    """Status de um agente"""
    LOCAL_ENABLED = "local_enabled"                # Agente localhost descoberto e habilitado
    LOCAL_DISABLED = "local_disabled"              # Agente localhost descoberto mas desabilitado
    REMOTE_ADDED = "remote_added"                  # Agente remoto adicionado manualmente (pode ser removido)
    OFFLINE = "offline"                            # Agente não está respondendo


class AgentInfo(BaseModel):
    """Informações estendidas sobre um agente"""
    agent_card: AgentCard
    status: AgentStatus
    last_seen: Optional[datetime.datetime] = None
    is_online: bool = True
    is_local: bool = True  # True para localhost, False para remoto
    enabled: bool = True
    can_be_removed: bool = False  # True apenas para agentes remotos
...
class ToggleAgentRequest(JSONRPCRequest):
    """Request para habilitar/desabilitar um agente"""
    method: Literal['agent/toggle'] = 'agent/toggle'
    params: dict  # {"agent_url": str, "enabled": bool}


class ToggleAgentResponse(JSONRPCResponse):
    """Response para toggle de agente"""
    result: dict | None = None  # {"success": bool, "message": str}


class RefreshAgentsRequest(JSONRPCRequest):
    """Request para forçar redescoberta de agentes"""
    method: Literal['agent/refresh'] = 'agent/refresh'


class RefreshAgentsResponse(JSONRPCResponse):
    """Response para refresh de agentes"""
    result: dict | None = None  # {"discovered_count": int, "message": str}
