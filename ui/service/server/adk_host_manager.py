...
from service.types import Conversation, Event, AgentInfo, AgentStatus
...
class ADKHostManager(ApplicationManager):
    ...
    def __init__(
        ...
        self._agents: dict[str, AgentInfo] = {}  # URL -> AgentInfo
        ...
        # Inicializar sistema de descoberta de agentes
        self._agent_discovery = AgentDiscovery(http_client)
        ...
        # Descobrir e registrar agentes automaticamente (sem bloquear inicialização)
        try:
            # Executar descoberta inicial em background
            import threading
            discovery_thread = threading.Thread(target=self._run_initial_discovery)
            discovery_thread.daemon = True
            discovery_thread.start()
        except Exception as e:
            print(f"Erro ao iniciar descoberta inicial: {e}")
    ...
    def _initialize_host(self):
        # Coletar apenas agentes habilitados para registrar no host
        enabled_agent_cards = []
        for agent_info in self._agents.values():
            if agent_info.enabled and agent_info.is_online:
                enabled_agent_cards.append(agent_info.agent_card)
        
        # Recriar host agent com agentes habilitados
        self._host_agent = HostAgent([], self._host_agent.httpx_client, self.task_callback)
        
        # Registrar agent cards habilitados
        for agent_card in enabled_agent_cards:
    ...
    def _run_initial_discovery(self):
        """Executa descoberta inicial em thread separada"""
        try:
            import asyncio
            ...
            # Executar descoberta
            loop.run_until_complete(self._auto_discover_agents())
            
            # Iniciar monitoramento periódico
            loop.run_until_complete(self._start_periodic_discovery())
            
        except Exception as e:
            print(f"Erro na descoberta inicial: {e}")
        finally:
            try:
                loop.close()
            except:
    ...
    async def _auto_discover_agents(self):
        """Descobre agentes automaticamente na inicialização"""
        try:
            discovered = await self._agent_discovery.discover_localhost_agents()
            for agent_card in discovered:
                await self._add_discovered_agent(agent_card)
            print(f"✅ Descobertos {len(discovered)} agentes automaticamente")
        except Exception as e:
            print(f"❌ Erro na descoberta automática: {e}")

    async def _start_periodic_discovery(self):
        """Inicia descoberta periódica a cada 5 minutos"""
        while True:
            try:
                ...
                print(f"Erro na descoberta periódica: {e}")

    async def _add_discovered_agent(self, agent_card: AgentCard):
        """Adiciona um agente local descoberto automaticamente"""
        url = agent_card.url
        
        if url not in self._agents:
            # Novo agente local descoberto
            agent_info = AgentInfo(
                agent_card=agent_card,
                status=AgentStatus.LOCAL_ENABLED,
                last_seen=datetime.datetime.now(),
                is_online=True,
                is_local=True,
                enabled=True,
                can_be_removed=False  # Agentes locais não podem ser removidos
            )
            self._agents[url] = agent_info
            print(f"🔍 Novo agente local descoberto: {agent_card.name} ({url})")
            # Reinicializar host com novos agentes
            self._initialize_host()
        else:
            # Agente já conhecido, atualizar status
            existing = self._agents[url]
            existing.last_seen = datetime.datetime.now()
            existing.is_online = True
            existing.agent_card = agent_card  # Atualizar dados

    async def _refresh_agents(self):
        """Força nova descoberta e atualiza status dos agentes"""
        try:
            # Descobrir agentes ativos
            discovered = await self._agent_discovery.discover_localhost_agents()
            discovered_urls = {agent.url for agent in discovered}
            
            # Adicionar novos agentes descobertos
            for agent_card in discovered:
                await self._add_discovered_agent(agent_card)
            
            # Marcar agentes não encontrados como offline
            for url, agent_info in self._agents.items():
                if url not in discovered_urls:
                    agent_info.is_online = False
    ...
    def register_agent(self, url):
        """Registra um agente remoto manualmente"""
        try:
            agent_data = get_agent_card(url)
            if not agent_data.url:
                agent_data.url = url
            
            # Determinar se é local ou remoto baseado na URL
            is_local = url.startswith('http://localhost:') or url.startswith('http://127.0.0.1:')
            
            agent_info = AgentInfo(
                agent_card=agent_data,
                status=AgentStatus.LOCAL_ENABLED if is_local else AgentStatus.REMOTE_ADDED,
                last_seen=datetime.datetime.now(),
                is_online=True,
                is_local=is_local,
                enabled=True,
                can_be_removed=not is_local  # Apenas agentes remotos podem ser removidos
            )
            ...
            print(f"Erro ao registrar agente {url}: {e}")

    def toggle_agent(self, url: str, enabled: bool) -> bool:
        """Habilita/desabilita um agente (apenas para agentes locais)"""
        if url in self._agents:
            agent_info = self._agents[url]
            
            # Apenas agentes locais podem ser habilitados/desabilitados
            if agent_info.is_local:
                agent_info.enabled = enabled
                
                # Atualizar status baseado no novo estado
                if enabled:
                    agent_info.status = AgentStatus.LOCAL_ENABLED
                else:
                    agent_info.status = AgentStatus.LOCAL_DISABLED
                
                self._initialize_host()
                return True
        return False

    def remove_agent(self, url):
        """Remove completamente um agente (apenas para agentes remotos)"""
        if url in self._agents:
            agent_info = self._agents[url]
            
            # Só permite remoção completa de agentes remotos
            if not agent_info.is_local and agent_info.can_be_removed:
                del self._agents[url]
                print(f"🗑️ Agente remoto removido: {agent_info.agent_card.name} ({url})")
            else:
                print(f"⚠️ Agente local não pode ser removido, apenas desabilitado: {url}")
                # Para agentes locais, apenas desabilita
                if agent_info.is_local:
    ...
    def agents(self) -> list[AgentInfo]:
        """Retorna lista de AgentInfo (não mais AgentCard)"""
